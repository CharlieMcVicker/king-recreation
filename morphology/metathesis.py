...
def add_h_in_final_cluster(pro_form: str) -> str:
    return re.sub("([^aeiouv]+)([aeiouv]?)$", r"\1h\2", pro_form)


def drop_h_in_final_cluster(pro_form: str) -> str:
    return re.sub("([^aeiouv]+)h([aeiouv]?)$", r"\1\2", pro_form)


def add_h_in_first_cluster(stem: str) -> str:
    return re.sub("^([aeiouv]?)([^aeiouv]+)", r"\1\2h", stem)


def drop_h_in_first_cluster(stem: str) -> str:
    return re.sub("^([aeiouv]?)([^aeiouv]+)h", r"\1\2", stem)


def metathesize_h(pro_form: str, stem: str) -> tuple[str, str]:
    """
...
def demetathesize_h(h_less_form: str, joined: str) -> str | None:
    """
