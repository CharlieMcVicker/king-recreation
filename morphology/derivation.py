...
from morphology.word_spec import WordSpec, SyntacticCategory
from morphology.morphology_types import Aspect, Person, Number, PronominalSet
...
def derive_pronouns(examples: list[tuple[str, WordSpec]]) -> list[tuple[PrePronominalConfig, PronominalConfig]]:
    """
                ...
                for d in [False, True]:
                    ...
                    for idx, (word, spec) in enumerate(examples):
                        if (
                            ...
                            and spec.aspect not in (Aspect.PERFECTIVE, Aspect.INFINITIVE)
                        ):
                    ...
                    if candidate_1st_set_a is not None:
                        aki = candidate_1st_set_a.startswith("aki") or candidate_1st_set_a.startswith("akhi")
                    else:
                        aki = False
                                ...
                                for s_type in StemType:
                                    for allow_h_metathesis in [False, True]:
                                        ...
                                        for uwa in uwa_opts:
                                            ...
                                            valid_configs.append((pre_config, pron_config))
