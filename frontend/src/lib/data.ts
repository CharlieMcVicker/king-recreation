...
export async function getMatchCounts() {
  return readCsv<any>(path.join(ARTIFACTS_DIR, 'analysis'), 'class_match_counts.csv');
}

export async function getVerbCoverage() {
  return readJson<any>(path.join(ARTIFACTS_DIR, 'analysis'), 'verb_coverage.json');
}

export async function getNearMisses() {
  return readCsv<any>(path.join(ARTIFACTS_DIR, 'analysis'), 'class_near_misses.csv');
}
