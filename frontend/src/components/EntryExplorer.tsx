...
import { CheckCircle, XCircle, ArrowRight, Info, BookOpen } from "lucide-react";
...
export default function EntryExplorer({
  ...
  const sortedMatches = [...matches].sort((a, b) => {
    // Sort by Scope (full first) then Class Order
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800 shadow-sm flex flex-col h-full overflow-hidden">
        <div className="p-4 border-b border-gray-200 dark:border-zinc-800 bg-gray-50/50 dark:bg-zinc-800/20 flex items-center justify-between">
          <h3 className="font-semibold text-sm">
            Matched Classes ({matches.length})
          </h3>
        </div>
            ...
            >
              <div className="flex items-center gap-3">
                <span
                  className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                    match.scope === "full" ? "bg-indigo-500" : "bg-amber-500"
                  }`}
                />
              ...
              <div className="flex items-center gap-2">
                <div className="flex flex-col items-end gap-1">
                  {match.scope === "ending" && (
                    <span className="text-[10px] uppercase font-bold text-amber-600 bg-amber-50 dark:bg-amber-900/20 px-1.5 py-0.5 rounded">
                      Near Miss
                    </span>
                  )}
                  {match.scope === "full" && (
                    <span className="text-[10px] uppercase font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 px-1.5 py-0.5 rounded">
                      Full Match
                    </span>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800 shadow-sm flex flex-col h-full overflow-hidden">
        ...
        {selectedMatch && selectedClassData ? (
          <div className="p-6 space-y-8 flex-1 overflow-y-auto">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-lg font-bold font-mono text-indigo-600 dark:text-indigo-400">
                  {selectedMatch.class}
                </h4>
              </div>
            ...
            <div className="space-y-4">
              <h5 className="text-[10px] font-bold uppercase text-gray-400 tracking-wider">
                Form Verification vs Class Pattern
              </h5>
            ...
            <div className="p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20 rounded-lg">
              <div className="flex gap-3">
                ...
                <div className="text-xs text-amber-800 dark:text-amber-200 leading-relaxed">
                  <strong>Verification Info:</strong> This view compares the
                  actual forms of <em>{corpusEntry.definition}</em> against the
                  patterns defined in Class{" "}
                  <strong>{selectedMatch.class}</strong>.
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 space-y-8 flex-1 overflow-y-auto">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <h4 className="text-lg font-bold font-serif text-gray-900 dark:text-white">
                  Corpus Forms
                </h4>
              </div>
              ...
              <div className="p-4 bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-900/20 rounded-lg">
                <div className="flex gap-3">
                  ...
                  <div className="text-xs text-indigo-800 dark:text-indigo-200 leading-relaxed">
                    Select a match from the left sidebar to compare these forms
                    against a specific class pattern.
                  </div>
                </div>
              </div>
            )}
