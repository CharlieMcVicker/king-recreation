import { getClasses, getInitialMatches, getCorpus } from "@/lib/data";
...
export default async function EntryPage({
  ...
  const corpusEntry = corpus.find(
    (c: any) => String(c.corpus_id) === String(corpusId)
  );
  ...
  const entryMatches = allMatches.filter(
    (m: any) => String(m.corpus_id) === String(corpusId)
  );
      ...
      <div className="flex flex-col gap-4 border-b border-gray-200 dark:border-zinc-800 pb-6">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider bg-gray-100 dark:bg-zinc-800 px-2 py-1 rounded">
              Lexical Entry
            </span>
          ...
          <h1 className="text-2xl md:text-3xl font-serif font-bold text-gray-900 dark:text-white leading-tight">
            {corpusEntry.definition || "Unknown Definition"}
          </h1>
