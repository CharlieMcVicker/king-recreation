...
The `artifacts/` directory contains all outputs generated by the analysis pipeline, organized into `data`, `reports`, `visualizations`, and `debug`.
...
- **Match Scope Filter**: Update `MatchExplorer.tsx` to include `reconstructs` in the `scopeFilter` dropdown.
- **Visual Indicators**: Add a visual distinction (e.g., color) for the `reconstructs` scope.
- **Root Consistency**: Add a row or indicator in the "Match Details" view to display "Root Consistency" status.
