---
id: doc-7
title: Reconstruction from Roots Specification
type: specification
created_date: '2026-06-10 16:18'
---# Reconstruction from Roots Specification

This specification governs the logic for programmatically reconstruct-generating surface forms from underlying roots and configuration flags.

---

## 1. Consonant Respelling Reform
...
### A. Root Extraction & Grade Selection
The process begins by reading the curated [validated_reconstructable_roots.csv](../../../../curated/validated_reconstructable_roots.csv) (or `derived_roots.csv` for initial runs) and reference [corpus.csv](../../../../artifacts/data/corpus.csv).

**Logic:**
1. **Input**: Iterate through verbs.
...
3. **Grade Selection**: For each target form, the engine selects the appropriate grade using `use_glottal_grade`:
...
### B. Reconstruction (Generation)
Implemented in [morphology/reconstruction.py](../../../../morphology/reconstruction.py), the generator functions as the inverse of the stem derivation and classification process.
...
3. **Middle Voice Application**: If `middle_voice` is configured, apply the middle voice transformation to the root (e.g. prefixing `ali-`, `ati-`) before prefix attachment (see [middle_voice.py](../../../../morphology/morphemes/middle_voice.py)).
...
- **H-Metathesis**: If `allow_h_metathesis` is used, aspiration moves to the prefix (e.g., `ka-` + `nh...` -> `khanh...`). Implemented in [metathesis.py](../../../../morphology/metathesis.py).
...
### A. Validation Logic
- **Input**: Generated candidate sets and [corpus.csv](../../../../artifacts/data/corpus.csv) (Reference).
- For each verb, check if the Reference form is present in the set of Generated forms.
- If all reference forms match at least one generated variant, the reconstruction is successful.
- **Consistency**: Verifies that the same class and root can generate all observed forms.

### B. Output Artifacts
- `reconstructable_verbs.json`: Fully serialized successfully reconstructed verbs for frontend use.
- [validated_reconstructable_roots.csv](../../../../curated/validated_reconstructable_roots.csv): The source of truth for user-validated roots and configurations.
