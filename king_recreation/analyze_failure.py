...
def analyze_failure(target_definition):
    ...
    if not row:
        print(
            json.dumps(
                {"error": f"Could not find definition '{target_definition}' in corpus."}
            )
        )
        return
    ...
    analysis = {"definition": target_definition, "forms": forms, "configurations": []}
                            ...
                            for fn, form_literals in possible_stems.items():
                                ...
                                if not is_h_dropping_set(pron_type):
                                    for (
                                        ...
                                        form_literals
                                    ):  # analyze_failure uses raw strings in list
                                        if s:
                                            candidates.add(s)
                                        ...
                                        for s in possible_stems[fn]:
                                            ...
                                            if use_strict:
                                                if is_strict_compatible(s, dropped):
                                                    is_valid = True
                                                elif is_compatible_with_vowel_restoration(
                                                    s, dropped
                                                ):
                                                    is_valid = True
                                            else:
                                ...
                                for fn in possible_stems:
                                    ...
                                    for s in possible_stems[fn]:
                                        ...
                                        for target in valid_candidates:
                                            ...
                                            pron_type = get_pronominal_set_name(
                                                fn, pron_config
                                            )
                                            is_h_drop = is_h_dropping_set(pron_type)
                                            ...
                                            if use_strict:
                                                ...
                                                if is_loose_compatible(s, target):
                                                    matched = True
                                                elif is_h_drop and is_loose_compatible(
                                                    s, drop_first_h(target)
                                                ):
                                                    matched = True
    ...
    def score_config(c):
        if c["outcome"] == "Success":
            return 3
        if c["outcome"] == "Inconsistent":
            return 2
        if c["outcome"] == "Form Failure":
            return 1
        return 0
