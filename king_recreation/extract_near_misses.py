...
def extract_near_misses(class_id, strictness):
    ...
    if not near_miss_results:
        print(f"No near misses found for class {class_id} with strictness {strictness}.")
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract near miss data for a specific class.")
    parser.add_argument("--class_id", required=True, help="Verb class ID (e.g., Ic)")
    parser.add_argument("--strictness", choices=['strict', 'loose'], default='loose', help="Match strictness")
