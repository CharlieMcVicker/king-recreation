...
*   If a mixed `row_spec` is selected, the interface must allow the user to select *both* the canonical stative derivation and the canonical eventful derivation.
...
1. **Introduce `RowSpec` Enum:** Define the `RowSpec` enum in the core models (e.g., `pure_stative`, `mixed_inf_only`, `mixed_imp_inf`, `eventful`). Temporarily update the pipeline to assume *every* row is `eventful`. This lays the structural groundwork without changing behavior.
2. **Refactor Aspect Class System:**
   ...
   * Write unit tests passing in multiple entries with the *same* `Aspect` (e.g., testing the system's ability to handle multiple `Aspect.INCOMPLETIVE` forms for statives, even if the current pipeline doesn't use that feature yet).
